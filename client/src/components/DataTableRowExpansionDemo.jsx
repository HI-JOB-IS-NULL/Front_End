...
const DataTableRowExpansionDemo = () => {
    ...
    const searchItem = (rowData) =>{
        ...
        axios({
            method: 'GET',
            url: `${ServeIP}/shop/productListByPName`,
            params: {
                    ...
                    <td style={{ "padding": "5px" }}>
                        {
                            data.orderStatus != "CANCEL"
                                ? <div>
                                    <ConfirmDialog visible={visible} onHide={() => setVisible(false)} message="Are you sure you want to Cancel?"
                                        header="Confirmation" icon="pi pi-exclamation-triangle" accept={() => accept(data.orderId)} reject={reject} />
                                    <Button onClick={() => setVisible(true)} icon="pi pi-check" label="Cancel Order" className="p-button-outlined p-button-danger" style={{ "padding": "revert" }} />
                                </div>
                                : null
                        }
                    </td>
